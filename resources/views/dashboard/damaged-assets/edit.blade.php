...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="post" action="/damagedassets/{{ $asetrusak->id }}" class="mb-5" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
                ...
                <select class="form-select" name="asset_id" id="nama" required autofocus>
                    @foreach ($assets as $asset)
                        @if (old('asset_id', $asetrusak->asset_id) == $asset->id)
            ...
            <div class="mb-3">
                ...
                <textarea type="text" name="kondisi" class="form-control @error('kondisi') is-invalid @enderror" id="kondisi"
                    rows="2" required>{{ old('kondisi', $asetrusak->kondisi) }}</textarea>
                @error('kondisi')
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="solusi" id="solusi" required autofocus>
                    ...
                    @else
                        <option value="{{ $asetrusak->solusi }}" hidden>{{ $asetrusak->solusi }}</option>
                        <option value="Perbaiki/Maintenance">Perbaiki/Maintenance</option>
                        <option value="Hapus Dari Sistem">Hapus Dari Sistem</option>
                    @endif
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="jumlah" class="form-control @error('jumlah') is-invalid @enderror"
                    id="jumlah" value="{{ old('jumlah', $asetrusak->jumlah) }}" required>
                @error('jumlah')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <button type="submit" class="btn tombol">Edit Aset Rusak</button>
